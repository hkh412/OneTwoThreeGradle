...
public class CommentRow extends LinearLayout {
	...
	public CommentRow(Context context, AttributeSet attrs) {
		...
		imgAuthor = (ImageView)view.findViewById(R.id.img_cmt_level);
	}
	...
	public void setCommentImg(String fullUrl) {
		AQuery aq = AQueryManager.getInstance(mContext).getAQuery();
		aq.id(imgAuthor).image(fullUrl);
	}
}
